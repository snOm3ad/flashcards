...
function Question({ question, showAnswer, setShowAnswer, setScore, meta }) {
  ...
  // NOTE: `timeLimit` will never change as specified in the api.
  //        But this is the only way to get the linter to shutup.
  useEffect(() => {
    totalTime.current = timeLimit;
  }, [timeLimit]);

  useEffect(() => {
    // only if the users asks to see the answer we will
    // calculate the score. So technically this effect
    // should not run when `time` changes. However, since
    // this function does not do any work it should not
...
function StudySession({ url }) {
  ...
  const hasBeenAnswered = index =>
    answeredFlashCards.length === 0 || index > answeredFlashCards.slice(-1)[0];
